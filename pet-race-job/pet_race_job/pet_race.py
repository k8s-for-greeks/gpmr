...
class PetRace(object):
  ...
  self.racers_positions_by_time

  def numpy_normal(racers_still_running_len):
    ...
    random_normal = numpy.random.normal(loc=0.0, scale=1.0, size=racers_still_running_len)
    return random_normal

  # TODO
  def save_normal(normal):

    return

  # TODO
  def save_racer_current(racer_guid):
  ...
  #TODO
  def save_racer_current_point(racer):
    return
  ...
  def run(self):
    ...
    while True:
      ...
      random_normal = numpy_normal(len(self.racers_still_running))
      save_normal(random_normal)
        for racer in self.racers_still_running:
          ...
          for racer in racers_finished_this_iteration:
            racers[racer]['total_time'] = calc_finish_time(racers[racer])
            racers[racer]['total_distance'] = current_racer_distance
            racers[racer]['finished'] = finished
            # TODO
