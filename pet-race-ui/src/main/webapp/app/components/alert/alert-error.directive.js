(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('gpmrApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-gpmrApp-error');
                var entityKey = httpResponse.headers('X-gpmrApp-params');
                if (errorHeader) {
